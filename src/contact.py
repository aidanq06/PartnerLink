...
def switch_to_contact_window(root, return_to_main):
    ...
    title_label = ctk.CTkLabel(left_frame, text="Contact Companies", font=("Roboto Medium", 40), fg_color="white", bg_color="white")
    title_label.pack(pady=20)

    # Input boxes for email, subject, and content
    email_input = ctk.CTkEntry(left_frame, placeholder_text="Company Email", width=400, height=35, font=("Roboto Medium", 18))
    email_input.pack(pady=5)

    subject_input = ctk.CTkEntry(left_frame, placeholder_text="Subject", width=400, height=35, font=("Roboto Medium", 18))
    subject_input.pack(pady=5)

    content_input = ctk.CTkTextbox(left_frame, width=400, height=150, font=("Roboto Medium", 15))
    content_input.pack(pady=(5,10))

    # Fetch companies and create radio buttons in the scrollable frame
    company_details = fetch_company_details()
    selected_company = tk.StringVar(value="")

    # Scrollable frame in the right frame for company selection
    scrollable_frame = ctk.CTkScrollableFrame(right_frame)
    scrollable_frame.pack(fill="both", expand=True, padx=20, pady=20)

    for company, email in company_details.items():
        radio_btn = ctk.CTkRadioButton(scrollable_frame, text=company, variable=selected_company, value=company, 
    ...
    confirm_button = ctk.CTkButton(left_frame, text="Send Email", text_color="white",hover_color="#444444", font=("Roboto Medium", 20), fg_color="black", command=on_send_email, width=250, height=45)
    ...
    def on_view_replies():
        ...
        scrollable_frame = ctk.CTkScrollableFrame(replies_frame)
        scrollable_frame.pack(fill="both", expand=True, padx=20, pady=20)
        ...
        if replies:
            for reply in replies:
                ...
                radio_btn = ctk.CTkRadioButton(scrollable_frame, font=("Roboto Medium", 18), text=option_text, variable=selected_reply, value=reply['id'],
                                            command=lambda: print(selected_reply.get()))
                radio_btn.pack(pady=5, fill='x')
        else:
            no_reply_label = ctk.CTkLabel(scrollable_frame, text="No replies found", font=("Roboto Medium", 20), fg_color="#dbdbdb", bg_color="white")
            no_reply_label.pack()
    ...
    replies = ctk.CTkButton(left_frame, text="View Replies", text_color="white", font=("Roboto Medium", 20),hover_color="#444444", fg_color="black", command=on_view_replies, width=250, height=45)
    replies.pack(pady=10)

    # Back button to return to the main menu
    back_button = ctk.CTkButton(left_frame, text="Back", text_color="white", font=("Roboto Medium", 20),hover_color="#444444", fg_color="black", command=return_to_main, width=250, height=45)
